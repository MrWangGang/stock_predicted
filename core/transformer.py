...
config = {
    'seq_length': 10,
    'input_size': 5,  # 仅保留5个原始特征，输入维度变为5
    'd_model': 512,
    'nhead': 256,
    'output_size': 1,
    'batch_size': 32,
...
def train_model(model, train_loader, val_loader, epochs, lr, threshold, best_model_path, scaler, close_index,
    ...
    for epoch in range(epochs):
        ...
        for X_batch, y_batch in train_loader:
            ...
            train_acc += calculate_accuracy(output.squeeze().cpu(), y_batch.cpu(), threshold, scaler, close_index)
            mse, rmse = calculate_mse_rmse(output.squeeze().cpu(), y_batch.cpu(), scaler, close_index)
            train_mse += mse
            train_rmse += rmse
        ...
        with torch.no_grad():
            for X_batch, y_batch in val_loader:
                ...
                loss = loss_fn(output.squeeze(), y_batch)
                val_loss += loss.item()
                val_acc += calculate_accuracy(output.squeeze().cpu(), y_batch.cpu(), threshold, scaler, close_index)
                mse, rmse = calculate_mse_rmse(output.squeeze().cpu(), y_batch.cpu(), scaler, close_index)
                val_mse += mse
                val_rmse += rmse
...
def plot_k_line_with_predictions(csv_path, predictions_real, seq_length, save_path):
    ...
    fig, axes = mpf.plot(k_data, type='candle', style='charles', volume=True, addplot=apds, returnfig=True)
    # 保存 K 线图
...
if __name__ == "__main__":
    ...
    model = SimpleTransformer(config['input_size'], config['d_model'], config['nhead'], config['output_size']).to(device)
